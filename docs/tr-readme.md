...
(`english`, `french`, `german`, `turkish`, `azerbaijani`, `kazakh`, `russian`, `chinese`, `arabic`, `greek`, `japanese`, `armenian`, `ukrainian`, `czech`, `polish`, `latvian`, `romanian`, `italian`, `spanish`, `portuguese`) toplam 20 dili destekleyen zaman kütüphanesi barındırır.
...
- Belirtilen bölge kodunun (`ro_RO`) zaman kütüphanesinde bulunan rafını döndürebilir.(İlk parametre tarih, ikinci parametre dil kodu olduğunda.)
```php
// Array
// (
...
- Belirtilen raf öğesini (`month_names`, `abbreviated_month_names`, `days_of_week`, `date_words`, `date_format`, `locale`) döndürebilir. (İlk parametre `null`, ikinci parametre dil kodu olduğunda.)
```php
...
- Zaman kütüphanesinin envanterini döndürebilir. (Parametresiz çalıştırıldığında.)
```php
// Array
// (
//     [english] => Array
...
Belirtilen url'nin domain adresini elde etmek için kullanılır, bu metot `$this->project_domain` değişkeninde ki domain varyasyonlarını destekler.
...
Kendisiyle paylaşılan metinden belirtilen sayı kadar cümleyi `.?!` işaretlerini ayraç olarak kullanarak parçalamaya ardından arasına boşluk koyarak birleştirerek geri döndürmeye yarar. Belirtilmesi zorunlu iki `string` türünde parametre alır.
...
çıktı:
```php
Ortaya çıkan gizemli bir virüs, tüm şehri etkisi altına alır.
```

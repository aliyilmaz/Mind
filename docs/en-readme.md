...
-   [getSunPhase](https://github.com/aliyilmaz/Mind/blob/main/docs/en-readme.md#getSunPhase)
-   [getMoonPhase](https://github.com/aliyilmaz/Mind/blob/main/docs/en-readme.md#getMoonPhase)
...
This method is used to compile and return the sun data returned by php's `date_sun_info()` command. It takes the following parameters.

- Latitude (required)
- Longitude (required)
- Timestamp (optional, if not sent `$this->timestamp` timestamp is used.)
- Timezone (optional, if not sent `$this->timezone` timestamp is used.)
...
This method is used to return the month data generated by mathematical calculations. It takes the following parameters.

- Timestamp (required)
- Timezone (optional, if not sent `$this->timezone` timezone is used.)
...
## astronomy()
This method returns the results of the `getSunPhase()` and `getMoonPhase()` methods that provide astronomical data.

To access solar data, the following parameters must be passed to the `sun` key.
- lat (required)
- lon (required)
- timestamp (optional, if not sent, the `$this->timestamp` timestamp is used. )
- timezone (optional, if not sent, the `$this->timezone` timestamp is used. )

To access lunar data, the following parameters must be passed to the `moon` key.
- timestamp (required)
- timezone (optional, if not sent, the `$this->timezone` timestamp is used. )

**Note:** `timestamp` parameters can also be specified in UNIX timestamp type.
